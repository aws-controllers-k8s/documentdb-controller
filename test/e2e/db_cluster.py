...
def wait_until(
    ...
    """Waits until a DB cluster with a supplied ID is returned from the DocumentDB API
...
def get(db_cluster_id):
    """Returns a dict containing the DB cluster record from the DocumentDB API.

    If no such DB cluster exists, returns None.
    """
    c = boto3.client('docdb')
    try:
...
def get_tags(db_cluster_arn):
    """Returns a dict containing the DB cluster's tag records from the DocumentDB API.
